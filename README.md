...
- [nostr-shard-signer](https://github.com/saintego/nostr-shard-signer) - A drop-in progressive `window.nostr` bridge. It provides Web2 social logins by deriving key shares on-the-fly inside an isolated Web3Auth MPC cross-origin iframe (the `nsec` is never exposed to the parent app) for newbies, while cleanly proxying to native NIP-07 extensions or NIP-46 bunkers (via fiatjaf's `window.nostr.js`) for power users.
